...
def names_match(scorer, player_name):
    ...
    if 'al-arab' in s_norm and 'alarab' in p_norm:
        return True
    if 'al-bulayhi' in s_norm and 'albulayhi' in p_norm:
        return True
    if 'al-taamari' in s_norm and 'al-taamari' in p_norm:
        return True
    if 'in-beom' in s_norm and 'inbeom' in p_norm:
        return True
    if 'sojka' in s_norm and 'sojka' in p_norm:
        return True
    if 'tim payne' in s_norm and 'timothy' in p_norm:
        return True
    if 'chris wood' in s_norm and 'christopher' in p_norm:
        return True
    if 'zico' in s_norm and 'mostafa' in p_norm:
        return True
    if 'trezeguet' in s_norm and 'mahmoud' in p_norm:
        return True
    if 'dani olmo' in s_norm and 'daniel' in p_norm:
        return True
    if 'taamari' in s_norm and 'suleiman' in p_norm:
        return True
    if 'barcola' in s_norm and 'abarcola' in p_norm:
        return True
    if 'kang-in' in s_norm and 'kangin' in p_norm:
        return True
    if 'nico gonzalez' in s_norm and 'nicolas' in p_norm and 'gonzalez' in p_norm:
        return True
    if 'vitinha' in s_norm and 'vitor machado' in p_norm:
        return True
    if 'gouiri' in s_norm and 'ghouiri' in p_norm:
        return True
    if 'baena' in s_norm and 'alejandro baena' in p_norm:
        return True
    if 'thelo aasgaard' in s_norm and 'thelonious' in p_norm:
        return True
    if 'casemiro' in s_norm and 'casimiro' in p_norm:
        return True
    if 'diney' in s_norm and 'edilson alberto' in p_norm:
        return True
    if 'azzedine' in s_norm and 'azz-eddine' in p_norm:
        return True
    if 'alrashdan' in s_norm and 'al-rashdan' in p_norm:
        return True
    if 'haydos' in s_norm and 'alhaydos' in p_norm:
        return True
    if 'schjelderup' in s_norm and 'schjelderup' in p_norm:
        return True
    if 'altamari' in s_norm and 'suleiman' in p_norm:
        return True
    if s_norm == 'isak' and 'hien' in p_norm:
        return False
    if 'm. araujo' in s_norm and 'ronald' in p_norm:
        return False
    if s_norm == 'hussein' and 'haydar' in p_norm:
        return False
    if 'cucho' in s_norm and 'juan camilo hernandez' in p_norm:
        return True
    if 'paqueta' in s_norm and 'lucas tolentino' in p_norm:
        return True
    if 'ihsan' in s_norm and 'ehsan' in p_norm and 'haddad' in p_norm:
        return True
        
    s_clean = s_norm.replace('-', ' ')
    p_clean = p_norm.replace('-', ' ')
    
    s_words = [w for w in s_clean.split() if len(w) > 2]
    p_words = [w for w in p_clean.split() if len(w) > 2]
    
    if not s_words: s_words = s_clean.split()
    if not p_words: p_words = p_clean.split()
    ...
    # For common short names like "McGinn" or "Isak", make sure we match correctly
    # If the scorer has only one word, e.g. "McGinn", and it is in p_words, that's a match
    if len(s_words) == 1 and s_words[0] in p_set:
        return True
    if len(p_words) == 1 and p_words[0] in s_set:
        return True
        
    # If they share at least 2 words (e.g. "Hwang In-beom" shares "hwang" and "inbeom" with "Inbeom Hwang")
    if len(s_set & p_set) >= 2:
        return True
    return False
...
def main():
    ...
    latest_match_date = max(m['date'] for m in completed_matches) if completed_matches else "2026-07-01"
    ...
    for p in players:
        ...
        row = {
            ...
            "last_verified": latest_match_date
        }
